...
module Scrapers
  ...
  class PremiereProduceOneApi
    ...
    ORDER_GUIDE_QUERY = 'query GetOrderGuideItems($restaurantUUID: uuid!, $supplierUUID: uuid!) { getOrderGuideItems(restaurant_id: $restaurantUUID, supplier_id: $supplierUUID) { order_guide_item_id variant_pack { external_item_id uuid item { category description display_name photo_url_list uuid __typename } metadata pack { unit unit_count uuid __typename } __typename } __typename } }'

    SEARCH_QUERY = 'query SearchItems($query: String!, $restaurantUUID: uuid!, $supplierUUID: uuid!, $fulfillmentDate: String) { searchVariantPacks(query: $query, restaurant_id: $restaurantUUID, supplier_id: $supplierUUID, fulfillment_date: $fulfillmentDate) { variant_pack { external_item_id uuid item { category description display_name photo_url_list uuid __typename } pack { unit unit_count uuid __typename } __typename } __typename } }'
