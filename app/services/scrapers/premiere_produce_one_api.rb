...
module Scrapers
  ...
  class PremiereProduceOneApi
    ...
    CREATE_ORDER_QUERY = 'mutation NewOrder_CreateOrder($deliveryDate: timestamptz!, $fulfillmentType: FulfillmentType!, $orderDomain: OrderDomain!, $restaurantUUID: uuid!, $supplierUUID: uuid!) { createOrder(delivery_date: $deliveryDate, fulfillment_type: $fulfillmentType, order_domain: $orderDomain, restaurant_id: $restaurantUUID, supplier_id: $supplierUUID) { order { uuid status placed_at restaurant_desired_delivery_time fulfillment_type orders_items { restaurant_display_name variants_pack { uuid external_item_id __typename } __typename } __typename } __typename } }'

    UPDATE_CART_QUERY = 'mutation NewOrder_UpdateCart($orderUUID: uuid!, $updatedItems: [UpdatedItemInput!]!) { updateCart(order_id: $orderUUID, updated_items: $updatedItems) { order { uuid status orders_items { restaurant_display_name order_item_prices { pack_quantity_at_order __typename } variants_pack { uuid external_item_id __typename } __typename } __typename } __typename } }'
